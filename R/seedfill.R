seedfill <- function(z, x=1, y=1, fcol=0, bcol=1, tol=1e-6) {
  ...
  ##if (abs(fcol - bcol) <=tol) stop("abs(fcol - bcol) <= tol, fill color must be different from border color")
  ...
  z <-.C("seedfill",as.integer(m),
                ...
                as.double(ffcol), 
                as.double(bcol),
		as.double(tol), PACKAGE="simecol")$z

  ## fix: then we replace ffcol with fcol
